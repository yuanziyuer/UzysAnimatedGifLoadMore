...
//    NSLog(@"state:%d",self.state);
    self.center = CGPointMake(roundf(self.center.x), roundf(self.scrollView.contentSize.height + (yOffset - self.originalBottomInset)/2 ));
    switch (_state) {
...
-(void)_actionStopState {
    self.state = UZYSGIFLoadMoreStateCanFinish;
